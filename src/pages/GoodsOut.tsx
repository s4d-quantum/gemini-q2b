  ...
  const loadOrders = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        `)
        .or('status.eq.processing,status.eq.confirmed,status.eq.complete')
  ...
  return (
    <div>
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Goods Out</h1>
          <p className="mt-2 text-sm text-gray-700">
            Manage sales orders ready for shipment
          </p>
        </div>
        <div>
          {/* No action button needed for Goods Out */}
        </div>
      </div>
